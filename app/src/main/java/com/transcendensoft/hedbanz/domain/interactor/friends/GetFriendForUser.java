 ...
 * that represents a use case for retrieving data related to a specific
 * * {@link com.transcendensoft.hedbanz.domain.entity.Friend}.
 *
 * @author Andrii Chernysh. E-mail: itcherry97@gmail.com
 *         Developed by <u>Transcendensoft</u>
 */
public class GetFriendForUser extends ObservableUseCase<Friend, Long> {
    private FriendsDataRepository mRepository;
    private PreferenceManager mPreferenceManger;

    @Inject
    public GetFriendForUser(ObservableTransformer schedulersTransformer,
    ...
    @Override
    protected Observable<Friend> buildUseCaseObservable(Long friendId) {
        return mRepository.getFriendForUser(
                friendId, mPreferenceManger.getUser().getId(), DataPolicy.API);
    }
}