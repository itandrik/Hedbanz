 ...
 */
public class GetFriends extends ObservableUseCase<List<Friend>, DataPolicy> {
    private FriendsDataRepository mRepository;
    private PreferenceManager mPreferenceManger;

    @Inject
    public GetFriends(ObservableTransformer schedulersTransformer,
    ...
    protected Observable<List<Friend>> buildUseCaseObservable(DataPolicy params) {
        return mRepository.getFriends(mPreferenceManger.getUser().getId(), params);
    }
}
