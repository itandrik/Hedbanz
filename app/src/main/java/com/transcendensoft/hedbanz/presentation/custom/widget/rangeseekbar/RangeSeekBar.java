...
/**
 * Range seekbar widget for rooms filtering.
 *
 * @author Andrii Chernysh. E-mail: itcherry97@gmail.com
 *         Developed by <u>Transcendensoft</u>
 */

public class RangeSeekBar extends View implements ViewTreeObserver.OnGlobalLayoutListener, ViewTreeObserver.OnScrollChangedListener {
    ...
    private float lastProgress1;
    private float lastProgress2;
    ...
    public RangeSeekBar(Builder builder) {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        p.mProgress1 = ta.getFloat(R.styleable.IndicatorSeekBar_isb_progress_1, p.mProgress1);
        p.mProgress2 = ta.getFloat(R.styleable.IndicatorSeekBar_isb_progress_2, p.mProgress2);
    ...
    private void initData() {
        ...
        if (p.mProgress1 < p.mMin) {
            p.mProgress1 = p.mMin;
        }
        if (p.mProgress1 > p.mMax) {
            p.mProgress1 = p.mMax;
        }
        if (p.mProgress2 < p.mMin) {
            p.mProgress2 = p.mMin;
        }
        if (p.mProgress2 > p.mMax) {
            p.mProgress2 = p.mMax;
        }
        ...
        if (p.mThumbDrawable == null) {
            ...
            int maxThumbWidth = ViewUtils.dpToPx(mContext, CUSTOM_DRAWABLE_MAX_LIMITED_WIDTH);
        ...
        if (p.mTickDrawable == null) {
            ...
            int maxTickWidth = ViewUtils.dpToPx(mContext, CUSTOM_DRAWABLE_MAX_LIMITED_WIDTH);
        ...
        if (needDrawText()) {
            ...
            mTextHeight = 0;
            mTextHeight += mRect.height() + ViewUtils.dpToPx(mContext, 2 * GAP_BETWEEN_SEEK_BAR_AND_BELOW_TEXT);
        }
        lastProgress1 = p.mProgress1;
        lastProgress2 = p.mProgress2;
    }

    private void calculateProgressTouchX1() {
        //progress
        float touchX = (p.mProgress1 - p.mMin) * mSeekLength / (p.mMax - p.mMin) + mPaddingLeft;
        calculateTouchX1(touchX);
    }

    private void calculateProgressTouchX2() {
        //progress
        float touchX = (p.mProgress2 - p.mMin) * mSeekLength / (p.mMax - p.mMin) + mPaddingLeft;
        calculateTouchX2(touchX);
    }

    float getTouchX1() {
        calculateProgressTouchX1();
        return mTouchX1;
    }

    float getTouchX2() {
        calculateProgressTouchX2();
        return mTouchX2;
    }
    ...
    private void initDefaultPadding() {
        if (!p.mClearPadding) {
            int normalPadding = ViewUtils.dpToPx(mContext, 16);
            if (getPaddingLeft() == 0) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(resolveSize(ViewUtils.dpToPx(mContext, 170), widthMeasureSpec), height + mTextHeight);
        initSeekBarInfo();
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        if (!mDrawAgain) {
            //progress
            float touchX1 = (p.mProgress1 - p.mMin) * mSeekLength / (p.mMax - p.mMin) + mPaddingLeft;
            calculateTouchX1(touchX1);

            float touchX2 = (p.mProgress2 - p.mMin) * mSeekLength / (p.mMax - p.mMin) + mPaddingLeft;
            calculateTouchX2(touchX2);
            mDrawAgain = true;
        }
        float thumbX1 = getThumbX(mTouchX1);
        float thumbX2 = getThumbX(mTouchX2);
        //draw BG track before range
        mStockPaint.setStrokeWidth(p.mBackgroundTrackSize);
        mStockPaint.setColor(p.mBackgroundTrackColor);
        canvas.drawLine(mSeekStart, mTrackY, thumbX1, mTrackY, mStockPaint);
        //draw Progress track
        mStockPaint.setStrokeWidth(p.mProgressTrackSize);
        mStockPaint.setColor(p.mProgressTrackColor);
        canvas.drawLine(thumbX1 + mThumbRadius, mTrackY, thumbX2, mTrackY, mStockPaint);
        //draw BG track after range
        mStockPaint.setStrokeWidth(p.mBackgroundTrackSize);
        mStockPaint.setColor(p.mBackgroundTrackColor);
        canvas.drawLine(thumbX2 + mThumbRadius, mTrackY, mSeekEnd, mTrackY, mStockPaint);

        //draw
        //draw tick
        drawTicks(canvas, thumbX1, thumbX2);
        //draw text below tick
        drawText(canvas);
        //drawThumbText
        //drawThumbText(canvas, thumbX);
        //drawThumb
        drawThumb(canvas, thumbX1);
        drawThumb(canvas, thumbX2);
    ...
    /*private void drawThumbText(Canvas canvas, float thumbX) {
        ...
        if (p.mThumbProgressStay) {
            canvas.drawText(getProgressString(p.mProgress), thumbX + p.mBackgroundTrackSize / 2.0f, mPaddingTop + mThumbTouchHeight + mRect.height() + ViewUtils.dpToPx(mContext, 2), mTextPaint);
        }
    }*/
    ...
    private void drawTicks(Canvas canvas, float thumbX1, float thumbX2) {
        ...
        for (int i = 0; i < mTextLocationList.size(); i++) {
            ...
            if (getThumbPosOnTick1() == i || getThumbPosOnTick2() == i) {
                continue;
            }
            if (p.mTickOnThumbLeftHide) {
                if (thumbX1 < locationX && thumbX2 > locationX) {
                    continue;
                }
            }
            ...
            int rectWidth = ViewUtils.dpToPx(mContext, 1);
            if (p.mTickDrawable != null) {
                if (mTickDraw == null) {
    ...
    private Bitmap getBitmapDraw(Drawable drawable, boolean isThumb) {
        ...
        int width;
        int height;
        int maxRange = ViewUtils.dpToPx(mContext, CUSTOM_DRAWABLE_MAX_LIMITED_WIDTH);
    ...
    private void drawText(Canvas canvas) {
        ...
        int gap = ViewUtils.dpToPx(mContext, 3);
    ...
    private float getThumbX(float touchX) {
        float mThumbXCache = touchX - p.mBackgroundTrackSize / 2.0f;
        float mThumbX;
    ...
    public int getThumbPosOnTick1() {
        if (p.mSeekBarType > 1) {
            return Math.round((mTouchX1 - mPaddingLeft) / mSeekBlockLength);
        } else {
            return -1;
        }
    }

    public int getThumbPosOnTick2() {
        if (p.mSeekBarType > 1) {
            return Math.round((mTouchX2 - mPaddingLeft) / mSeekBlockLength);
        } else {
            return -1;
        }
    }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (isTouchSeekBar(mX, mY) && !p.mForbidUserSeek && isEnabled()) {
                    if (isTouchThumb1(mX)) {
                        if (mListener != null) {
                            mListener.onStartTrackingTouch(this, getThumbPosOnTick1());
                        }
                        refreshSeekBarThumb1(event);
                        thumb = 1;
                        return true;
                    }
                    if (isTouchThumb2(mX)) {
                        if (mListener != null) {
                            mListener.onStartTrackingTouch(this, getThumbPosOnTick2());
                        }
                        refreshSeekBarThumb2(event);
                        thumb = 2;
                        return true;
                    }
                }
            ...
            case MotionEvent.ACTION_CANCEL:
                if (mListener != null) {
                    mListener.onStopTrackingTouch(this, getProgress1(), getProgress2());
                }
                mIsTouching = false;
                invalidate();
                if (p.mShowIndicator) {
    ...
    private void setListenerThumb1(boolean formUserTouch) {
        if (mListener != null) {
            mListener.onProgressChanged(this, getProgress(p.mProgress1), getProgressFloat(p.mProgress1), formUserTouch);
            if (p.mSeekBarType > 1) {
                int thumbPosOnTick = getThumbPosOnTick1();
    ...
    private void setListenerThumb2(boolean formUserTouch) {
        if (mListener != null) {
            mListener.onProgressChanged(this, getProgress(p.mProgress2), getProgressFloat(p.mProgress2), formUserTouch);
            if (p.mSeekBarType > 1) {
                int thumbPosOnTick = getThumbPosOnTick2();
                if (p.mTextArray != null && thumbPosOnTick < (p.mTextArray.length)) {
                    mListener.onSectionChanged(this, thumbPosOnTick, String.valueOf(p.mTextArray[thumbPosOnTick]), formUserTouch);
                } else {
                    mListener.onSectionChanged(this, thumbPosOnTick, "", formUserTouch);
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        bundle.putFloat("isb_progress1", p.mProgress1);
        bundle.putFloat("isb_progress2", p.mProgress2);
        return bundle;
    }
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        if (state instanceof Bundle) {
            ...
            p.mProgress1 = bundle.getFloat("isb_progress1");
            p.mProgress2 = bundle.getFloat("isb_progress2");
    ...
    private void refreshSeekBarThumb1(MotionEvent event) {
        calculateTouchX1(adjustTouchX(event));
        calculateProgress1();
        mIsTouching = true;
        if (lastProgress1 != p.mProgress1) {
            setListenerThumb1(true);
        }
        invalidate();
    }

    private void refreshSeekBarThumb2(MotionEvent event) {
        calculateTouchX2(adjustTouchX(event));
        calculateProgress2();
        mIsTouching = true;
        if (lastProgress2 != p.mProgress2) {
            setListenerThumb2(true);
        }
        invalidate();
    }
    ...
    private void calculateProgress1() {
        lastProgress1 = p.mProgress1;
        p.mProgress1 = p.mMin + (p.mMax - p.mMin) * (mTouchX1 - mPaddingLeft) / mSeekLength;
    }

    private void calculateProgress2() {
        lastProgress2 = p.mProgress2;
        p.mProgress2 = p.mMin + (p.mMax - p.mMin) * (mTouchX2 - mPaddingLeft) / mSeekLength;
    }

    private void calculateTouchX1(float touchX) {
        int touchBlockSize = Math.round((touchX - mPaddingLeft) / mSeekBlockLength);
        if(mSeekBlockLength * touchBlockSize + mPaddingLeft <= mTouchX2){
            mTouchX1 = mSeekBlockLength * touchBlockSize + mPaddingLeft;
        }
    }

    private void calculateTouchX2(float touchX) {
        int touchBlockSize = Math.round((touchX - mPaddingLeft) / mSeekBlockLength);
        if(mSeekBlockLength * touchBlockSize + mPaddingLeft >= mTouchX1){
            mTouchX2 = mSeekBlockLength * touchBlockSize + mPaddingLeft;
        }
    }

    private boolean isTouchSeekBar(float mX, float mY) {
        if (mFaultTolerance == -1) {
            mFaultTolerance = ViewUtils.dpToPx(mContext, 5);
        }
    ...
    public int getProgress1() {
        return Math.round(p.mProgress1);
    }

    public int getProgress2() {
        return Math.round(p.mProgress2);
    }
     ...
     */
    public synchronized void setProgress1(float progress) {
        if (progress < p.mMin) {
            p.mProgress1 = p.mMin;
        } else if (progress > p.mMax) {
            p.mProgress1 = p.mMax;
        } else {
            p.mProgress1 = progress;
        }
        setListenerThumb1(false);
        float touchX = (p.mProgress1 - p.mMin) * mSeekLength / (p.mMax - p.mMin) + mPaddingLeft;
        calculateTouchX1(touchX);
        initSeekBarInfo();
        postInvalidate();
    ...
    public synchronized void setProgress2(float progress) {
        if (progress < p.mMin) {
            p.mProgress2 = p.mMin;
        } else if (progress > p.mMax) {
            p.mProgress2 = p.mMax;
        } else {
            p.mProgress2 = progress;
        }
        setListenerThumb2(false);
        float touchX = (p.mProgress2 - p.mMin) * mSeekLength / (p.mMax - p.mMin) + mPaddingLeft;
        calculateTouchX2(touchX);
        initSeekBarInfo();
        postInvalidate();
    ...
    public synchronized Builder getBuilder() {
        ...
        mRawParams.mProgress1 = p.mProgress1;
        mRawParams.mProgress2 = p.mProgress2;
     ...
     */
    public synchronized float getProgressFloat(float progress) {
        return getProgressFloat(1, progress);
    }
    ...
    private void apply(BuilderParams p) {
        ...
        initData();
        initSeekBarInfo();
        setProgress1(p.mProgress1);
        setProgress2(p.mProgress2);
        requestLayout();
        /*if (mIndicator != null) {
            if (mIndicator.isShowing()) {
                mIndicator.forceHide();
     ...
     *
     * @param listener OnRangeSeekBarChangeListener
     */
    public void setOnSeekChangeListener(@NonNull OnRangeSeekBarChangeListener listener) {
        this.mListener = listener;
    }

    public boolean isTouchThumb1(float mX) {
        float rawTouchX = getTouchX1();
        return rawTouchX - p.mThumbSize / 2f <= mX && mX <= rawTouchX + p.mThumbSize / 2f;
    }

    public boolean isTouchThumb2(float mX) {
        float rawTouchX = getTouchX2();
        return rawTouchX - p.mThumbSize / 2f <= mX && mX <= rawTouchX + p.mThumbSize / 2f;
    }
    ...
    public interface OnRangeSeekBarChangeListener {
         ...
         */
        void onProgressChanged(RangeSeekBar seekBar, int progress, float progressFloat, boolean fromUserTouch);
         ...
         */
        void onSectionChanged(RangeSeekBar seekBar, int thumbPosOnTick, String textBelowTick, boolean fromUserTouch);
         ...
         *                       no ticks on seek bar, so the thumbPosition will be always -1.
         */
        void onStartTrackingTouch(RangeSeekBar seekBar, int thumbPosOnTick);
         ...
         * @param seekBar The SeekBar in which the touch gesture began
         */
        void onStopTrackingTouch(RangeSeekBar seekBar, int progressLeft, int progressRight);
    }

    public static class Builder {
        BuilderParams p;
        RangeSeekBar indicatorSeekBar;
         ...
         */
        public RangeSeekBar build() {
            return new RangeSeekBar(this);
        }
         ...
         * @return Builder
         */
        public Builder setProgress1(float progress) {
            p.mProgress1 = progress;
            return this;
        }

        public Builder setProgress2(float progress) {
            p.mProgress2 = progress;
            return this;
        }
        ...
        public Builder setBackgroundTrackSize(int backgroundTrackSize) {
            p.mBackgroundTrackSize = ViewUtils.dpToPx(p.mContext, backgroundTrackSize);
            return this;
        }
        ...
        public Builder setTextSize(int textSize) {
            p.mTextSize = ViewUtils.dpToPx(p.mContext, textSize);
            return this;
        }
        ...
        public Builder setTickSize(int tickSize) {
            p.mTickSize = ViewUtils.dpToPx(p.mContext, tickSize);
            return this;
        }
        ...
        public Builder setProgressTrackSize(int progressTrackSize) {
            p.mProgressTrackSize = ViewUtils.dpToPx(p.mContext, progressTrackSize);
            return this;
        }
        ...
        public Builder setThumbWidth(int thumbWidth) {
            p.mThumbSize = ViewUtils.dpToPx(p.mContext, thumbWidth);
            return this;
        }
        ...
        public Builder setIndicatorTextSize(int indicatorTextSize) {
            p.mIndicatorTextSize = ViewUtils.dpToPx(p.mContext, indicatorTextSize);
            return this;
        }
        ...
        Builder setSeekBar(RangeSeekBar indicatorSeekBar) {
